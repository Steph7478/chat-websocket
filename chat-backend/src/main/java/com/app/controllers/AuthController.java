...
public class AuthController {
        ...
        public Mono<ResponseEntity<Map<String, String>>> login(
                ...
                Mono<ResponseEntity<Map<String, String>>> original = Mono.zip(
                                jwtService.generate(username),
                                refreshTokenService.create(username))
                                .map(tuple -> {
                                        exchange.getResponse().addCookie(CookieFactory.createAuthCookie(tuple.getT1()));
                                        exchange.getResponse()
                                                        .addCookie(CookieFactory.createRefreshCookie(tuple.getT2()));
